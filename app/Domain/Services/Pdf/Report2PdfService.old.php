...
class Report2PdfServiceOld
{
    ...
    public function __construct(EvaluationRepository $evaluationRepository)
    {
        ...
        $this->pdf->AddFont('Roboto', 'B', 'Roboto-Bold.php');
    ...
    private function generateMajors(StudentResult $result)
    {
        ...
        foreach ($result->getMajorResults() as $majorResult) {
            ...
            $this->blue();
            $this->pdf->SetFont('Roboto', 'B', $this->majorFontSize);
            $this->pdf->SetAlpha(1);
    ...
    private function generateIac(ArrayCollection $iacs, BranchResult $branchResult)
    {
        if (count($iacs) > 0) {
            ...
            $oTable = new PdfTable($this->pdf);

            $oTable->setStyle("b", 'Roboto', "B", 10, implode(',', self::BLUE));
            $oTable->setStyle("p", 'Roboto', "", 9, implode(',', self::BLUE));
            $oTable->setStyle("sm", 'Roboto', "", 8, implode(',', self::BLUE));
            $oTable->setStyle('i', 'NotosIcon', '', 11, implode(',', self::BLUE));

            $oTable->initialize(array($this->iacTextWidth, $this->iacIconWidth, $this->iacIconWidth, $this->iacCommentWidth));
            $oTable->setRowConfig(array(
                'BACKGROUND_COLOR' => false
            ));

            $aHeader = [
                ['TEXT' => utf8_decode(ucfirst($branchResult->getName())), 'TEXT_ALIGN' => 'L'],
            ...
            foreach ($iacs as $iac) {
                $this->generateIacGoals($iac, $oTable);
            }
    ...
    private function generateIacGoals(IacResult $iac, PdfTable $oTable)
    {
        ...
        foreach ($iac->getGoals() as $goal) {
            ...
            $aRow = array();
            $aRow[0]['TEXT'] = '<p>' . utf8_decode($goal->getText()) . '</p>';
            $aRow[0]['ALIGN'] = 'L';
            $aRow[1]['TEXT'] = $goal->isAchieved() ? '<i>' . $this->fontmap['CHECK'] . '</i>' : '';
            $aRow[2]['TEXT'] = $goal->isPractice() ? '<i>' . $this->fontmap['CHECK'] . '</i>' : '';
            $aRow[3]['TEXT'] = '<sm>' . utf8_decode($goal->getComment()) . '</sm>';
            $aRow[3]['ALIGN'] = 'L';
    ...
    public function Footer(StudentResult $studentResult)
    {
        ...
        $this->pdf->SetX(($fnW - $this->pdf->pageWidth()) / 2);
