...
namespace App\Pdf\NtPdf\Table;


use App\Pdf\NtPdf\Multicell;
use App\Pdf\NtPdf\Table\Cell\Image;
use App\Pdf\NtPdf\Table\Cell\NtAbstract;
use App\Pdf\NtPdf\Table\Cell\TableEmpty;
use App\Pdf\NtPdf\Table\Cell\TableInterface;
use App\Pdf\NtPdf\Table\Cell\TableMulticell;
use App\Pdf\Pdf;
...
class Table
{
    ...
    protected $aTypeMap = array(
        'EMPTY' => TableEmpty::class,
        'MULTICELL' => TableMulticell::class,
        'IMAGE' => Image::class
    );
    ...
    public function __construct( $oPdf )
    {
        ...
        $this->oMulticell = new Multicell( $oPdf );
    ...
    protected function getCellObject( $data = null )
    {
        if ( null === $data )
        {
            $oCell = new TableMulticell( $this->oPdf );
        }
        ...
        else
        {
            ...
            /** @var $oCell TableInterface */
            $oCell->setProperties( $data );
        }

        if ( $oCell instanceof TableMulticell )
        {
            /** @var $oCell TableMulticell */
     ...
     * @param int $opacity
     */
    public function setStyle( $tag, $family, $style, $size, $color, $opacity = 1 )
    {
        $this->oMulticell->setStyle( $tag, $family, $style, $size, $color, $opacity );
    }
